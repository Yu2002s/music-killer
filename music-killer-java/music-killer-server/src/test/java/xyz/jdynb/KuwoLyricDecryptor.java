...
public class KuwoLyricDecryptor {
    ...
    public static String decodeLyrics(byte[] buf, boolean isGetLyricx) throws Exception {
        // 检查内容格式
        String header = new String(buf, 0, Math.min(10, buf.length), "UTF-8");
        ...
        byte[] bufStr = Base64.getDecoder().decode(base64Str);
        int bufStrLen = bufStr.length;
        byte[] output = new byte[bufStrLen];
    ...
    public static void main(String[] args) {
        try {
            ...
            byte[] responseData = HttpUtil.createGet(url).execute().bodyBytes();
