...
public class NuxtParser {
    ...
    public void testNuxtParser() throws Exception {
        try (Context context = Context.newBuilder("js")
                .allowAllAccess(true)
                .build()) {

            Value value = context.eval("js", parseNuxtState("https://www.kuwo.cn"));
            Value data = value.getMember("data");
            if (data.hasArrayElements()) {
                Value firstDataElement = data.getArrayElement(0);
                // Value banners = firstDataElement.getMember("banners");
            }
        }
    }
}